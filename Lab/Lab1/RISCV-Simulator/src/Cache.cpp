...
uint8_t sample_table1[4096];
uint8_t sample_table2[4096];
uint8_t sample_table3[4096];
...
std::vector<SamplerEntry> sampler(12);
...
void Cache::setByte(uint32_t addr, uint8_t val, uint32_t *cycles, uint64_t pc)
{
  ...
  if ((blockId = this->getBlockId(addr)) != -1)
  {
    ...
    if (!this->writeBack)
    {
      this->writeBlockToLowerLevel(this->blocks[blockId], pc);
      this->statistics.totalCycles += this->policy.missLatency;
    }
    if (cycles)
      *cycles = this->policy.hitLatency;
  ...
  {
    this->loadBlockFromLowerLevel(addr, cycles, pc);
...
void Cache::printStatistics()
{
  ...
  printf("Hit rate: %f\n", (float)this->statistics.numHit / (float)this->statistics.numMiss);
  if (this->lowerCache != nullptr)
  {
    printf("---------- LOWER CACHE ----------\n");
...
void Cache::loadBlockFromLowerLevel(uint32_t addr, uint32_t *cycles, uint64_t pc)
{
  ...
  if (this->lowerCache == nullptr)
  {
    ...
    if ((EntryId = getEntryId(tag)) != -1)
    {
      sample_table1[(sampler[EntryId].trace + sampler[EntryId].tag) % 4096]++;
      sample_table1[(sampler[EntryId].trace & sampler[EntryId].tag) % 4096]++;
      sample_table1[(sampler[EntryId].trace ^ sampler[EntryId].tag) % 4096]++;
    }
  }
  ...
  { // write back to memory
    this->writeBlockToLowerLevel(replaceBlock, pc);
    this->statistics.totalCycles += this->policy.missLatency;
  }
...
uint32_t Cache::getReplacementBlockId(uint32_t begin, uint32_t end, uint64_t pc)
{
  ...
  for (uint32_t i = begin; i < end; ++i)
  {
    ...
    {
      // printf("A dead block is replaced.\n");
      return i;
    }
  }
