...
public:
  ...
  void setByte(uint32_t addr, uint8_t val, uint32_t *cycles = nullptr, uint64_t pc = 0);
...
private:
  ...
  void initCache();
  void loadBlockFromLowerLevel(uint32_t addr, uint32_t *cycles = nullptr, uint64_t pc = 0);
  uint32_t getReplacementBlockId(uint32_t begin, uint32_t end, uint64_t pc = 0);
  void writeBlockToLowerLevel(Block &b, uint64_t pc = 0);
